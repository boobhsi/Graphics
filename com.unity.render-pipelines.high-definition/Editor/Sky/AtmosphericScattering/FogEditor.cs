            ...
            if (volumetricLightingAvailable)
            {
                ...
                if (isInAdvancedMode)
                {
                    ...
                    if (m_Anisotropy.value.floatValue != 0.0f)
                    {
                        ...
                        EditorGUILayout.HelpBox("When this slider is not 0, the anisotropy effect significantly increases the resource intensity of volumetric fog.", MessageType.Info, wide: true);
                    }
                }
